...
ax.scatter(sl_data['heightIn'],sl_data['weightLb'])
...
lm.fit(sl_data.heightIn[:,np.newaxis], sl_data.weightLb)

print ('Intercept is ' + str(lm.intercept_) + '\n')

print ('Coefficient value of the height is ' + str(lm.coef_) + '\n')

print ('The socre is  ' + str(lm.score(sl_data.heightIn[:,np.newaxis], sl_data.weightLb)) + '\n')
fig, ax = plt.subplots(1, 1)
ax.scatter(sl_data.heightIn,sl_data.weightLb)
ax.plot(sl_data.heightIn,lm.predict(sl_data.heightIn[:, np.newaxis]),
color = 'red')
ax.set_xlabel('Height')
ax.set_ylabel('Weight')
plt.show()
...
fig, ax = plt.subplots(1, 1)
ax.scatter(sl_data.heightIn, sl_data.ageYear)
ax.set_xlabel('height')
ax.set_ylabel('age')
plt.show()
'''
